...
def simple_analysis():
    ...
    """
    print("GCC-PHAT Analysis for MP3 Files")
    print("=" * 60)
    ...
    if not os.path.exists(reference_file):
        ...
        return
    if not os.path.exists(delayed_file):
        print(f"File not found: {delayed_file}")
        return
    
    # Find delay
    delay, correlation = find_delay_between_mp3s(
        reference_file, 
        delayed_file, 
        max_tau=5.0,      # Maximum expected delay in seconds
        analysis_length=30 # Analyze first 30 seconds
    )
    
    if delay is not None:
        print(f"\nSUCCESS!")
        print(f"The delayed file is {abs(delay):.3f} seconds behind the reference file")
        if delay < 0:
            print(f"(The 'delayed' file is {abs(delay):.3f} seconds AHEAD)")
